Module KruskalsAlgorithm
    Function KruskalsSimplified(limits() As Integer, delay As Integer, showMazeGeneration As Boolean, pathColour As ConsoleColor, backGroundColour As ConsoleColor)
        ...
        While edges.Count > 0 'edgeWeights.Count > 0
        ...
        Return returnpath
    End Function
    Function KruskalsTrue(limits() As Integer, delay As Integer, showMazeGeneration As Boolean, pathColour As ConsoleColor, backGroundColour As ConsoleColor)
        Dim tempLimits() = {limits(0), limits(1), limits(2), limits(3)}
        If backGroundColour <> ConsoleColor.Black Then DrawBackground(backGroundColour, tempLimits)
        Dim cellSet As New Dictionary(Of Cell, Integer)
        Dim availableCells As New List(Of Cell)
        Dim setNumber = 1
        Dim returnpath As New List(Of Node)
        Dim edgeWeights As New PriorityQueue(Of Node)
        Dim r As New Random
        For y = limits(1) To limits(3) Step 2
            For x = limits(0) + 3 To limits(2) Step 4
